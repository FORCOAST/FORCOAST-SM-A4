...
zip ${pwd}/add.zip -j $pwd/ERDAPP_temp_day_C/$YearForecast1$MonthForecast1${DayForecast1}T000000000Z.tif
curl -u "admin:#33f0rc0ast" -XPOST -H "Content-type:application/zip" -T ${pwd}/add.zip https://forecoast.apps.k.terrasigna.com/geoserver/rest/workspaces/test/coveragestores/surf_temp_daily_avg_north_sea_a4/file.imagemosaic?recalculate=nativebbox.latlonbbox
rm ${pwd}/add.zip

zip ${pwd}/add.zip -j $pwd/ERDAPP_temp_day_C/$YearForecast2$MonthForecast2${DayForecast2}T000000000Z.tif
curl -u "admin:#33f0rc0ast" -XPOST -H "Content-type:application/zip" -T ${pwd}/add.zip https://forecoast.apps.k.terrasigna.com/geoserver/rest/workspaces/test/coveragestores/surf_temp_daily_avg_north_sea_a4/file.imagemosaic?recalculate=nativebbox.latlonbbox
rm ${pwd}/add.zip

zip ${pwd}/add.zip -j $pwd/ERDAPP_temp_day_C/$YearForecast3$MonthForecast3${DayForecast3}T000000000Z.tif
curl -u "admin:#33f0rc0ast" -XPOST -H "Content-type:application/zip" -T ${pwd}/add.zip https://forecoast.apps.k.terrasigna.com/geoserver/rest/workspaces/test/coveragestores/surf_temp_daily_avg_north_sea_a4/file.imagemosaic?recalculate=nativebbox.latlonbbox
rm ${pwd}/add.zip